...
# Join DPD ingredient names to openFDA shortage records; matches are sparse
# by design (naming conventions differ across the two datasets).
FDA_FEATURES = [
...
def _load_raw(db_path: Path) -> pd.DataFrame:
    ...
    # Load base features plus FDA columns (available in panel, not in FEATURES).
