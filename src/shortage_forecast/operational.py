...
Usage:
    python -m shortage_forecast.operational
"""
...
def score_heuristic_single(test: pd.DataFrame) -> np.ndarray:
    """Crude ranker: sort by shortages_prior_12m, deterministic tiebreak.
...
def main() -> None:
    ...
    for k in TOP_K_OPERATIONAL:
        ...
        def lift(target: float, base: float = blend_mean) -> str:
            if target <= 0:
                return "  n/a"
            return f"{(base - target) / target:+.1%}"
